<template>
  <div class="border">
    <el-dialog title="更新变量" :visible.sync="ifshow" @open="openDialog" :before-close="closeDialog">
      <el-form :inline="true" :model="variableForm" :rules="rules" ref="variableForm" label-width="100px">
        ...
        <el-row type="flex" justify="center">
          ...
          <el-col :span="30">
            <el-form-item>
              <el-input v-model="variableForm.id" type="hidden"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row type="flex" justify="center">
...
export default {
  ...
  methods: {
    openDialog () {
      this.variableForm = clone(this.variable)
    },
    closeDialog () {
      ...
      this.$refs['variableForm'].validate((valid) => {
        if (valid) {
          this.$confirm('确认更新吗？')
            .then(async () => {
              try {
                const res = await this.$http.put('/variables', this.variableForm)
                if (res.code === '200') {
