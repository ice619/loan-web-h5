<template>
  <div class="border">
    ...
    <el-table ref="iosCompanySignTable" :data="tableData" border stripe highlight-current-row @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="channel" label="投放渠道" header-align="center" align="center" min-width="80"/>
...
export default {
  ...
  methods: {
    ...
    async list () {
      ...
      try {
        const res = await this.$http.post('/market-channel-detail/page', params)
        if (res.code === '200') {
    ...
    async removeBanner (row) {
      ...
      this.$confirm('确认删除吗？', '提示', {type: 'warning'}).then(async () => {
        try {
          const res = await this.$http.delete(`/market-channel-detail/${selectIdsStr}`)
          if (res.code === '200') {
