<template>
  <div class="border">
    <el-dialog title="新增市场渠道" :visible.sync="ifshow" @open="openDialog" :before-close="closeDialog" width="45%">
  ...
  methods: {
    ...
    save: debounce(300, function () {
      this.$refs['entryForm'].validate(async (valid) => {
        if (valid) {
          try {
            const res = await this.$http.post('/market-channel', this.entryForm)
            if (res.code === '200') {
