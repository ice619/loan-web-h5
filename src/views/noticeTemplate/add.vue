<template>
  <div class="border">
    <el-dialog title="新增消息模板" :visible.sync="ifshow" @open="openDialog" :before-close="closeDialog">
...
export default {
  ...
  methods: {
    openDialog () {
      this.noticeTemplateForm = clone(this.noticeTemplateInitForm)
    },
    closeDialog () {
      ...
      this.$refs['noticeTemplateForm'].validate(async (valid) => {
        ...
        if (valid) {
          try {
            ...
            const res = await this.$http.post('/management/notice-template', this.noticeTemplateForm)
            if (res.code === '200') {
