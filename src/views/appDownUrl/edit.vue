<template>
  <div class="border">
    <el-dialog title="编辑" :visible.sync="ifshow" @open="openDialog" :before-close="closeDialog" width="40%">
        ...
        <el-row type="flex" justify="left">
          <el-col :span="30">
            <el-form-item label="马甲包名称" prop="waistcoat" :rules="[{ required: true, message: '请输入马甲包名称', trigger: 'change' }]">
              <el-input v-model="entryForm.waistcoat" placeholder="马甲包名称" clearable style="width: 350px"/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row type="flex" justify="left">
          <el-col :span="30">
            <el-form-item label="下载链接" prop="downUrl">
              <el-input type="textarea" v-model="entryForm.downUrl" rows="4" style="width: 350px"/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row type="flex" justify="left">
  ...
  methods: {
    ...
    save: debounce(300, function () {
      this.$refs['entryForm'].validate(async (valid) => {
        if (valid) {
          try {
            const res = await this.$http.put('/app-down-url', this.entryForm)
            if (res.code === '200') {
