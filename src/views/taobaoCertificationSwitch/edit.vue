<template>
  <div class="border" style="width: 100%">
    <el-dialog title="编辑" :visible.sync="ifshow" @open="openDialog" :before-close="closeDialog" width="30%">
...
export default {
  props: {
    'ifshow': Boolean,
    'taobaoCertificationSwitchWindow': Object
  },
  data () {
    return {
      ruleForm: {},
  ...
  methods: {
    openDialog () {
      this.ruleForm = clone(this.taobaoCertificationSwitchWindow)
    },
    closeDialog () {
    ...
    save: debounce(300, function () {
      this.$refs['ruleForm'].validate(async (valid) => {
        if (valid) {
          try {
            const res = await this.$http.post('/management/taobao-certification-switch', this.ruleForm)
            if (res.code === '200') {
