...
export default {
  ...
  methods: {
    openDialog () {
      this.increaseButtonForm = clone(this.increaseButtonInitForm)
    },
    closeDialog () {
    ...
    save: debounce(300, function () {
      this.$refs['increaseButtonForm'].validate(async (valid) => {
        if (valid) {
          try {
            const res = await this.$http.post('/increase-button-config', this.increaseButtonForm)
            if (res.code === '200') {
