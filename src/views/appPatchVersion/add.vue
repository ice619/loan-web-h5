...
export default {
  ...
  methods: {
    openDialog () {
      this.appPatchVersionForm = clone(this.appPatchVersionInitForm)
    },
    closeDialog () {
    ...
    save: debounce(300, function () {
      this.$refs['appPatchVersionForm'].validate(async (valid) => {
        if (valid) {
          try {
            const res = await this.$http.post('/management/app-patch-version', this.appPatchVersionForm)
            if (res.code === '200') {
